...
## v0.9.1 — Bug Fixes and Improvements
...
**実装内容**:
- シーンリストの動的更新
  - 設定ウィンドウが開かれるたびにシーン一覧を更新
- 日本語ロケールの改善
  - 期間ラベルを「秒数」から「秒」に変更
  - 文字エンコーディング問題の修正
- 配信連動機能
  - OBS 配信開始時にプラグインを自動有効化（認証済みの場合）
  - OBS 配信停止時にプラグインを自動無効化

---

## v0.9.2 — Critical Bug Fix
...
**実装内容**:
- シーン選択の保持機能
  - シーン追加・削除後に設定ウィンドウを開いてもルール設定が保持される
  - データ損失の防止

---

## v0.9.3 — Authentication Error Feedback
...
**実装内容**:
- 認証エラーフィードバック
  - 認証情報未設定時にログインを試みた場合、エラーメッセージを表示
  - 認証設定画面への誘導
  - 英語・日本語のローカライズ対応

---

## v0.9.4 — Manual Scene Revert
...
**実装内容**:
- 手動シーン復帰ボタン
  - シーン切替中に「すぐにシーンを戻す」ボタンを表示
  - 配信者が自分の判断でタイマーを待たずに即座に復帰可能
  - State が Switched の時のみ表示
  - 英語・日本語のローカライズ対応
  - 既存の State Machine 設計と統合
